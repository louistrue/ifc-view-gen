...
export class SectionBox {
  ...
  /**
   * Create nice visual helpers for the section box
   */
  private createVisuals(bounds: { min: THREE.Vector3; max: THREE.Vector3 }): void {
    ...
    // Create a semi-transparent plane at the top cut
    const planeWidth = bounds.max.x - bounds.min.x
    const planeDepth = bounds.max.z - bounds.min.z
    const planeGeometry = new THREE.PlaneGeometry(planeWidth * 1.01, planeDepth * 1.01)
    ...
    this.cutPlane.rotation.x = -Math.PI / 2 // Horizontal plane
